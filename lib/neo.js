...
neo.retrieveNodeById = function (id, callback) {
...
neo.retrieveRelationshipById = function (id, callback) {
  db.getRelationshipById(id, function (err, node) {
    if (err) return callback(err);
    callback(null, node);
  });
};

neo.retrieveNodeByHash = function (hash, callback) {
  redis.getId(hash, function (id) {
    db.getNodeById(id, function (err, node) {
      if (err) return callback(err);
      callback(null, node);
    });
...
neo.test = function (callback) {
  ...
  node.save(function (err, node) {
      if (err) {
          ...
          console.log('Node saved to database with id:', node.id);
          callback(node.id);
          _self.retrieveNodeById(123, function (err, to) {
